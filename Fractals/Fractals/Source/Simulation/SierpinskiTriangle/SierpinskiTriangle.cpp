...
SierpinskiTriangle::SierpinskiTriangle(int width, int height, unsigned int seed) :
	Simulation(width, height, seed) { };

void SierpinskiTriangle::Initialize(int width, int height, unsigned int seed)
{
	...
	sierpinskiShader = make_unique<ComputeShader>("SierpinskiTriangle", width, height);
	sierpinskiShader->SetTextureBinding("dataTexture", texture->GetId());
	sierpinskiShader->SetUniform("size", width, height);
}

void SierpinskiTriangle::Restart()
{
	texture->Clear();
}

void SierpinskiTriangle::Execute()
{
	...
	sierpinskiShader->SetUniform("gradientAxis", FractalSettings::GradientAxis);
