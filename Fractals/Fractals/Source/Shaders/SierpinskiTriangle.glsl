...
bool ComputeSierpinski(dvec2 fractalPosition)
{
    double y = fractalPosition.y;
    double x = fractalPosition.x - (y / 2);
...
{
    double gradientPosition = gradientRepetition * (
        (fractalPosition.x * (1 - int(gradientAxis))) +
        (fractalPosition.y * int(gradientAxis))
    );
