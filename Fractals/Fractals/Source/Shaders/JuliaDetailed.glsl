...
void ComputeJulia(dvec2 fractalPosition, inout uint iterations, out float height)
{
    ...
    while (dot(criticalPoint, criticalPoint) < 650 && iterations < maxIterations)
    {
		...
		criticalPoint = ComplexMul(criticalPoint, criticalPoint) + constant;
        iterations++;
    }

    dvec2 normal = ComplexDiv(criticalPoint, derivative);
...
void main()
{
    ...
    ComputeJulia(fractalPosition, iterations, height);
